...
- **V5 — the clock, which is not a veto.** `defer` is the third outcome and is not a failure: a check still running, or a conflict axis GitHub has not finished computing, is re-derived next pass in silence. `mergeGateTimeoutSeconds` from the head commit's date is what stops that being forever. **An expired defer is not a `no`**: the deferring rows are never reclassified at expiry — they still read `defer` and still name what they are waiting on — and the clock gets its own row, verdict `note`, emitted whenever anything defers and never when nothing does. What changes past the bound is the kind on the handover's first line, not any row's verdict. **Nothing in the gate is exempt from that clock.** It used to defer on the walkthrough's rate-limit marker, ahead of every veto and outside the clock; that is gone. Its defence — *a throttled pull request keeps a stale verdict* — was contradicted by its own test fixture, a current High verdict naming the head, whose correct escalation the defer suppressed; a stale verdict is the chain's to route, and where the chain and the gate disagree about which commit a verdict names, V1 escalates whether or not the marker is there. The full argument sits where the test was, in `risk_gate`. Throttling is a fact about the reviewer, not the verdict, and the chain's head-scoped status read is the loop's only surface for it. The cost is deliberate: the gate can merge a minimal verdict that names this commit while CodeRabbit is throttled.
