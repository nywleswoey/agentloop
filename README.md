...
- **V1 — CodeRabbit's verdict.** Its merge-risk block must name a commit abbreviation that is a prefix of the head, and put the level at exactly *minimal*. An unrecognised level or an unparseable line escalates — the tripwire for CodeRabbit changing shape. A block that is missing entirely never reaches here: that is one of the two routes into *unreviewed*, which is nudged and bounded first.
...
| `stalled` | a CodeRabbit command was triggered and CodeRabbit never reported inside its bound | CodeRabbit's installation and seats |
